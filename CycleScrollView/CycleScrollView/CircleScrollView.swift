...
let imageFromResource = 0
let imageFromNetwork = 1

class CircleScrollView: UIView, UIScrollViewDelegate {
    /*********************************** Property ****************************************/
    //MARK:- Property -
    
    var contentScrollView: UIScrollView!
    var imageFrom = imageFromNetwork
    ...
    var imageUrlArray: [String]? {
        ...
        didSet {
            //这里用了强制拆包，所以不要把imageUrlArray设为nil
    ...
    var delegate: CircleScrollViewDelegate?
    ...
    private func setUpCircleView() {
        ...
        if imageFrom == imageFromNetwork {
            contentScrollView.scrollEnabled = !(imageUrlArray!.count == 1)
        } else {
            contentScrollView.scrollEnabled = !(imageArray.count == 1)
        }
...
//        nextImageView.contentMode = UIViewContentMode.ScaleAspectFill
        ...
        //设置分页指示器(如果以后需要扩展设置指示器的位置可以在这里进行修改)
        let itemsCount = imageFrom == imageFromNetwork ? imageUrlArray!.count : imageArray.count
        self.pageIndicator = UIPageControl(frame: CGRectMake(self.frame.size.width/2 - 20 * CGFloat(itemsCount)/2, self.frame.size.height - 20, 20 * CGFloat(itemsCount), 20))
        pageIndicator.hidesForSinglePage = true
        pageIndicator.numberOfPages = itemsCount
        pageIndicator.backgroundColor = UIColor.clearColor()
        self.addSubview(pageIndicator)
        
        //设置计时器
        if self.timer == nil && (self.imageArray?.count > 1 || self.imageUrlArray?.count > 1)
        {
    ...
    private func setScrollViewOfImage(){
        if imageFrom == imageFromResource {
            ...
            let currentImageUrl: String = self.imageUrlArray![self.indexOfCurrentImage]
            self.currentImageView.sd_setImageWithURL(NSURL(string: currentImageUrl))
            let nextImageUrl = self.imageUrlArray![self.getNextImageIndex(indexOfCurrentImage: self.indexOfCurrentImage)]
            self.nextImageView.sd_setImageWithURL(NSURL(string: nextImageUrl))
            let lastImageUrl = self.imageUrlArray![self.getLastImageIndex(indexOfCurrentImage: self.indexOfCurrentImage)]
            self.lastImageView.sd_setImageWithURL(NSURL(string: lastImageUrl))
    ...
    func scrollViewDidEndDecelerating(scrollView: UIScrollView) {
        ...
        //重置计时器
        if timer == nil && (self.imageArray?.count > 1 || self.imageUrlArray?.count > 1)
        {
            self.timer = NSTimer.scheduledTimerWithTimeInterval(TimeInterval, target: self, selector: "timerAction", userInfo: nil, repeats: true)
        }
    }
...
@objc protocol CircleScrollViewDelegate {
    /**
    *  点击图片的代理方法
    *
